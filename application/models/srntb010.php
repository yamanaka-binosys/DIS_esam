...
class Srntb010 extends CI_Model {
	...
	function get_srntb010_data($condition_data){
		log_message('debug',"========== srntb010 get_srntb010_data start ==========");
...
//		$edbn = $condition_data['edbn'];
		...
		log_message('debug',"========== srntb010 get_srntb010_data end ==========");
		return $result_data;
	}
	
	function update_srntb010_data($record_data)
	{
		...
		return $this->db->update('srntb010', $record_data, array('jyohonum' => $jyohonum));
	}
	
	function insert_srntb010_data($record_data)
	{
		unset($record_data['jyohonum']);
		unset($record_data['edbn']);
	...
	function delete_srntb010_data($jyohonum,$edbn){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		
		$sql .= "DELETE FROM srntb010";
		//$sql .= " WHERE jyohonum = '{$jyohonum}'";
		$sql .= " WHERE jyohonum = ?";
		$sql .= " ;";
	...
	function get_checkar_data($shbn,$ymd)
	{
		...
		// SQL文作成
		$sql .= " SELECT DISTINCT checker_edbn FROM srntb010 WHERE  shbn = ? AND ymd = ? ";
		log_message('debug',"\$sql = $sql");
		// SQL実行
