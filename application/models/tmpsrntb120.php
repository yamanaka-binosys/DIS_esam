...
class Tmpsrntb120 extends CI_Model {
	...
	/**
	 */
	function get_tmpsrntb120_data($condition_data){
		log_message('debug',"========== tmpsrntb120 get_tmpsrntb120_data start ==========");
		...
		log_message('debug',"========== tmpsrntb120 get_tmpsrntb120_data end ==========");
		return $result_data;
	}
	
	function insert_tmpsrntb120_data($record_data){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		$result_data = NULL; // 戻り値
...
//		$sql .= substr($value_string,0,-1);
	...
	function update_tmpsrntb120_data($jyohonum,$edbn,$aiteskcd,$aitesk_name,$aiteskrank){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		
		$sql .= " UPDATE tmpsrntb120";
		$sql .= "  SET aiteskcd = ?,";
		$sql .= "      aitesknm = ?,";
		$sql .= "      aiteskrank = ?,";
	...
	function delete_tmpsrntb120_data($jyohonum,$edbn,$shbn){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
	...
	function get_max_jyohonum()
	{
		...
		// SQL文作成
		$sql .= " SELECT MAX(jyohonum) AS jyohonum FROM tmpsrntb120;";
		log_message('debug',"\$sql = $sql");
		// SQL実行
		$query = $this->db->query($sql);
		// 取得確認
