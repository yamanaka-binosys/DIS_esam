...
class Srntb030 extends CI_Model {
	 ...
	 * 
	 */
	function get_srntb030_data($condition_data){
		log_message('debug',"========== srntb030 get_srntb030_data start ==========");
...
//		$edbn = $condition_data['edbn'];
		...
		log_message('debug',"========== srntb030 get_srntb030_data end ==========");
		return $result_data;
	}
	
	function update_srntb030_data($record_data)
	{
		...
		return $this->db->update('srntb030', $record_data, array('jyohonum' => $jyohonum));
	}
	
	function insert_srntb030_data($record_data)
	{
		unset($record_data['jyohonum']);
		unset($record_data['edbn']);
	...
	function delete_srntb030_data($jyohonum,$edbn){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		
		$sql .= "DELETE FROM srntb030";
		$sql .= " WHERE jyohonum = ?";
		$sql .= " ;";
	...
	function get_checkar_data($shbn,$ymd)
	{
		...
		// SQL文作成
		$sql .= " SELECT DISTINCT checker_edbn FROM srntb030 WHERE  shbn = ? AND ymd = ? ";
		log_message('debug',"\$sql = $sql");
		// SQL実行
  	...
  	function st_et_check($shbn, $ymd, $st, $et)
	{
		...
		// SQL文作成
		$sql .= " SELECT shbn, ymd, sthm, edhm FROM srntb030 WHERE shbn = ? AND ymd = ? ";
		log_message('debug',"\$sql = $sql");
		// SQL実行
		$query = $this->db->query($sql,array($shbn, $ymd));
		// 取得確認
