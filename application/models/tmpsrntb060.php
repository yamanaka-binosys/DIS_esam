...
class Tmpsrntb060 extends CI_Model {
	 ...
	 * 
	 */
	function get_tmpsrntb060_data($condition_data){
		log_message('debug',"========== tmpsrntb060 get_tmpsrntb060_data start ==========");
		...
		log_message('debug',"========== tmpsrntb060 get_tmpsrntb060_data end ==========");
		return $result_data;
	}
	
	function insert_tmpsrntb060_data($record_data){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		$result_data = NULL; // 戻り値
	...
	function update_tmpsrntb060_data($record_data){
	}
	
	function delete_tmpsrntb060_data($jyohonum,$edbn,$shbn){
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
	...
	function get_max_jyohonum()
	{
		...
		// SQL文作成
		$sql .= " SELECT MAX(jyohonum) AS jyohonum FROM tmpsrntb060;";
		log_message('debug',"\$sql = $sql");
		// SQL実行
		$query = $this->db->query($sql);
		// 取得確認
