...
class Srntb020 extends CI_Model {
	 ...
	 * 
	 */
	function get_srntb020_data($condition_data){
		log_message('debug',"========== srntb020 get_srntb020_data start ==========");
...
//		$edbn = $condition_data['edbn'];
		...
		log_message('debug',"========== srntb020 get_srntb020_data end ==========");
		return $result_data;
	}
	
	function update_srntb020_data($record_data)
	{
		...
		return $this->db->update('srntb020', $record_data, array('jyohonum' => $jyohonum));
	}
	
	function insert_srntb020_data($record_data)
	{
		unset($record_data['jyohonum']);
		unset($record_data['edbn']);
	...
	function delete_srntb020_data($jyohonum,$edbn){
		log_message('debug',"========== srntb020 delete_srntb020_data start ==========");
		// 初期化
		$sql = ""; // sql_regcase文字列
		$query = NULL; // SQL実行結果
		
		$sql .= "DELETE FROM srntb020";
		//$sql .= " WHERE jyohonum = '{$jyohonum}'";
		$sql .= " WHERE jyohonum = ?";
		$sql .= " ;";
		...
		if($query){
			log_message('debug',"========== srntb020 delete_srntb020_data end ==========");
			return TRUE;
		}else{
			log_message('debug',"========== srntb020 delete_srntb020_data end ==========");
			return FALSE;
	...
	function get_checkar_data($shbn,$ymd)
	{
		...
		// SQL文作成
		$sql .= " SELECT DISTINCT checker_edbn FROM srntb020 WHERE  shbn = ? AND ymd = ? ";
		log_message('debug',"\$sql = $sql");
		// SQL実行
  	...
  	function st_et_check($shbn, $ymd, $st, $et)
	{
		...
		// SQL文作成
		$sql .= " SELECT shbn, ymd, sthm, edhm FROM srntb020 WHERE shbn = ? AND ymd = ? ";
		log_message('debug',"\$sql = $sql");
		// SQL実行
		$query = $this->db->query($sql,array($shbn, $ymd));
		// 取得確認
