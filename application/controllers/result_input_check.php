...
class Result_input_check extends MY_Controller {

	function index($select_day = NULL){
		try{
			log_message('debug',"========== controllers result_input_check index start ==========");
			// 初期化
			$data_no = "";
			$group_count = 0;
			$data = NULL;
...
//			$data = $this->common_manager->init(SHOW_PLAN_C);
			...
			foreach ($_POST as $key => $value) {
				if($key === 'select_day' OR $key === 'kakninshbn' OR $key === 'kakninshnm' OR $key === 'move_day' OR $key === 'action' OR $key === 'check_hold' ){
					continue;
				}
			...
			log_message('debug',"========== controllers result_input_check index end ==========");
			$this->display($html_string);
		}catch(Exception $e){
			// エラー処理
			$this->load->view('/parts/error/error.php',array('errcode' => 'result_input_check-index'));
			//$this->error_view($e);
		}
	}
	...
	function display($data = NULL){
		try{
			log_message('debug',"========== controllers result_input_check display start ==========");
			log_message('debug',"========== controllers result_input_check display end ==========");
			// 表示処理
			$this->load->view(MY_VIEW_RESULT_INPUT_CHECK, $data, FALSE);
			return;
		}catch(Exception $e){
			// エラー処理
			$this->load->view('/parts/error/error.php',array('errcode' => 'result_input_check-display'));
			//$this->error_view($e);
		}
	}
	...
	function honbu_html($data = NULL,$no = NULL){
		...
		if(isset($data['seiykniyo_'.$no])){
			$content_data .= "成約内容\n".$data['seiykniyo_'.$no]."\n";
		}
		if(isset($data['fseiykniyo_'.$no])){
			$content_data .= "不成約内容\n".$data['fseiykniyo_'.$no]."\n";
		}
		if(isset($data['horyuniyo_'.$no])){
			$content_data .= "保留内容\n".$data['horyuniyo_'.$no]."\n";
		}
		if(isset($data['sonotaniyo_'.$no])){
			$content_data .= "その他内容\n".$data['sonotaniyo_'.$no]."\n";
		}
		if(!is_null($content_data)){
			$html_string .= "<pre>".$content_data."</pre>\n";
		}else{
			$html_string .= "<pre></pre>\n";
		}
		$html_string .= "</td>\n";
	...
	function tenpo_html($data = NULL,$no = NULL){
		...
		$html_string .= "</tr>\n";
		$html_string .= "<tr>\n";
		$html_string .= "<th colspan=\"2\" >項目</td>\n";
		$html_string .= "<th  >内容</td>\n";
		$html_string .= "</tr>\n";
		$html_string .= "<tr>\n";
		...
		$html_string .= "<td colspan=\"2\" class=\"item-list\"><div>".$item_name."</div></td>\n";
		$html_string .= "<td class=\"content-list\"  >\n";
		if(isset($data['sdn_niyo_'.$no])){
			$html_string .= "<pre >".$data['sdn_niyo_'.$no]."</pre>\n";
		}else{
			$html_string .= "<pre ></pre>\n";
		}
		$html_string .= "</td>\n";
	...
	function office_html($data = NULL,$no = NULL){
		...
		if(isset($data['sntsagyo_'.$no]) && $data['sntsagyo_'.$no] != "" ){
			...
			switch ($data['sagyoniyo_'.$no]) {
				case '001':
					log_message('debug',"aaaaa".$data['sagyoniyo_'.$no]);
					$item_name .= "月次商談資料作成";
					break;
				case '002':
					$item_name .= "半期提案資料作成";
	...
	function gyousya_html($data = NULL,$no = NULL){
		...
		$html_string .= "</tr>\n";
		$html_string .= "<tr>\n";
		$html_string .= "<td colspan=\"2\" class=\"item-list\" ></td>\n";
		$html_string .= "<td class=\"content-list\"  >\n";
...
/* End of file Result_input_check.php */
/* Location: ./application/controllers/result_input_check.php */
