...
class Checker_search_unit extends MY_Controller {
	/**
	 * 部署検索画面 unitとなっているが部署検索
	 *
	 * @access	private
	 * @param	none
	 * @return	none
	...
	function index($edbn = NULL)
	{
		try
		{
			...
			{
				$session_data = array(
					'busyo_search_shbn' => $_POST['busyo_list']
				);
				...
				if(isset($_POST['b_search']) OR isset($_POST['n_search']))
				{
					...
					$data['search_result_busyo_table'] = $this->table_manager->search_unit_set_b_table($shbn,NULL,$post['honbucd'],substr($_POST['bucd'],5));     // 部署検索
				}else{
					$data['search_result_busyo_table'] = $this->table_manager->search_unit_set_b_table($shbn);     // 部署検索
					$data['search_b_name_list']        = $this->table_manager->search_unit_set_list_table($shbn,$_POST); // 部署名リスト
				}
			}
			// 画面表示
			$this->display($data);
	...
	function display($data)
	{
		...
		{
			// 表示処理
			$this->load->view(MY_VIEW_CHECKER_SEARCH_UNIT, $data);
		}catch(Exception $e){
			// エラー処理
	...
	function search($shbn,$data = NULL)
	{
		try
		{
			...
			$data['search_result_busyo_table'] = $this->table_manager->search_unit_set_b_table($shbn,$post); // 部署検索
			$data['search_b_name_list']        = $this->table_manager->search_unit_set_list_table($shbn,$post);    // 部署名リスト
			return $data;
		}catch(Exception $e){
			// エラー処理
			//$this->error_view($e);
  ...
  function select_item_list()
  {
    try
    {
      ...
      $data['search_result_busyo_table'] = $this->table_manager->search_unit_set_b_table($shbn,NULL,$honbu); // 部署検索
      //リストデータ出力
      ...
      // エラー処理
     $this->load->view('/parts/error/error.php',array('errcode' => 'CHECKER-SEARCH-UNIT-SELECT-ITEM-LIST'));
      //$this->error_view($e);
    }
  }
